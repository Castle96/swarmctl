...
async fn describe_node(
    ...
    let node = nodes
        .into_iter()
        .find(|n| {
            n.id.as_ref() == Some(&name.to_string())
                || n.spec.as_ref().and_then(|s| s.name.as_ref()) == Some(&name.to_string())
        })
    ...
    match output {
        OutputFormat::Table => {
            ...
            if let Some(manager_status) = &node.manager_status {
                ...
                println!(
                    "Manager Address:\t{}",
                    manager_status.addr.as_ref().unwrap_or(&"".to_string())
                );
            }
            ...
            if let Some(description) = &node.description {
                ...
                if let Some(platform) = &description.platform {
                    println!(
                        "Platform:\t{}",
                        platform.architecture.as_ref().unwrap_or(&"".to_string())
                    );
            ...
            println!(
                "Created:\t{}",
                node.created_at.as_ref().unwrap_or(&"".to_string())
            );
            println!(
                "Updated:\t{}",
                node.updated_at.as_ref().unwrap_or(&"".to_string())
            );
        }
        ...
        OutputFormat::Table => {
            ...
            if let Some(task_template) = &spec.task_template {
                    ...
                    && let Some(limits) = &resources.limits
                {
                    println!(
                        "CPU Limit:\t{} cores",
                        limits.nano_cpus.unwrap_or(0) as f64 / 1_000_000_000.0
                    );
            ...
            if let Some(endpoint) = &spec.endpoint_spec
                && let Some(ports) = &endpoint.ports
            {
                println!("Ports:");
                for port in ports {
                    println!(
                        "\t{}:{} -> {}",
                        port.published_port.unwrap_or(0),
                        port.target_port.unwrap_or(0),
                        port.protocol
                            .as_ref()
                            .unwrap_or(&bollard::models::EndpointPortConfigProtocolEnum::TCP)
                            .as_ref()
                    );
                }
            }
            ...
            println!(
                "Created:\t{}",
                service.created_at.as_ref().unwrap_or(&"".to_string())
            );
            println!(
                "Updated:\t{}",
                service.updated_at.as_ref().unwrap_or(&"".to_string())
            );
        }
