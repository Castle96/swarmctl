...
pub async fn run(client: &DockerClient) -> Result<()> {
    ...
    if let Some(spec) = &swarm.spec {
        println!(
            "  \x1b[1mCluster Name:\x1b[0m   {}",
            spec.name.as_ref().unwrap_or(&"unknown".to_string())
        );
    }

    println!(
        "  \x1b[1mCluster ID:\x1b[0m     {}",
        swarm.id.as_ref().unwrap_or(&"unknown".to_string())
    );

    println!(
        "  \x1b[1mCreated At:\x1b[0m    {}",
        swarm.created_at.unwrap_or_default()
    );
    println!(
        "  \x1b[1mUpdated At:\x1b[0m    {}",
        swarm.updated_at.unwrap_or_default()
    );
