...
use crate::api::port::{
    get_available_ports, get_port_summary, list_service_ports, list_used_ports,
};
    ...
    match output {
        OutputFormat::Table => {
            ...
            if show_available {
                ...
                let available =
                    get_available_ports(client.inner(), start, end, protocol.as_deref()).await?;
...
fn print_ports_table(ports: &[ServicePortInfo]) {
    ...
    let unique_services = ports
        .iter()
        .map(|p| &p.service_name)
        .collect::<std::collections::HashSet<_>>()
        .len();
    println!(
        "  \x1b[1mTotal:\x1b[0m \x1b[32m{}\x1b[0m ports mapped across \x1b[36m{}\x1b[0m services",
        ports.len(),
        unique_services
    );
    println!();
...
fn print_available_ports(available: &[u16], start: u16, end: u16) {
    ...
    println!(
        "  \x1b[1;33mAvailable Ports\x1b[0m (Range: {} - {})",
        start, end
    );
    println!(
        "{}",
    ...
    println!(
        "  \x1b[1mTotal Available:\x1b[0m \x1b[32m{}\x1b[0m ports",
        available.len()
    );
    println!();
}
...
pub async fn run_tui(client: &DockerClient) -> Result<()> {
    ...
    println!(
        "  \x1b[1;37m│\x1b[0m  \x1b[32mTCP\x1b[0m Ports Used:   \x1b[1m{}\x1b[0m",
        summary.used_tcp.len()
    );
    println!(
        "  \x1b[1;37m│\x1b[0m  \x1b[33mUDP\x1b[0m Ports Used:   \x1b[1m{}\x1b[0m",
        summary.used_udp.len()
    );
    println!(
        "  \x1b[1;37m│\x1b[0m  \x1b[36mTotal Mappings:\x1b[0m \x1b[1m{}\x1b[0m",
        summary.port_mappings.len()
    );
    println!("  \x1b[1;37m│\x1b[0m");
    println!("  \x1b[1;37m└──────────────────────────────────────────────┘\x1b[0m");
    println!();
    ...
    println!(
        "  \x1b[1;37m┌─ Service Port Mappings ────────────────────────────────────────────────────┐\x1b[0m"
    );
    println!("  \x1b[1;37m│\x1b[0m");
    for (i, mapping) in summary.port_mappings.iter().enumerate().take(10) {
        ...
        println!(
            "  \x1b[1;37m│\x1b[0m  {:>3}. {:<20} \x1b[32m{:>5}\x1b[0m -> \x1b[36m{}\x1b[0m ({}) [{}]",
            i + 1,
            svc_trunc,
            pub_port,
            target,
    ...
    if summary.port_mappings.len() > 10 {
        println!(
            "  \x1b[1;37m│\x1b[0m  ... and {} more mappings",
            summary.port_mappings.len() - 10
        );
    }
    println!("  \x1b[1;37m│\x1b[0m");
    println!(
